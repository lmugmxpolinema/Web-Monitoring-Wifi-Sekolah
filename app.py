...
app = Flask(__name__)
app.secret_key = 's0t0kudus_secret_key_monitoring_2025'  # Secret key untuk session
...
@app.route('/api/record-ont-history', methods=['POST'])
def record_ont_history():
    """API untuk menyimpan rekor jumlah ONT aktif"""
    try:
        ...
        try:
            ...
            records = []
        # Only record once every 10 minutes
        MIN_INTERVAL = 600  # seconds
        # Determine last recorded timestamp
        last_ts = None
        if records:
            try:
        ...
        new_ts = None
        try:
            new_ts = datetime.fromisoformat(entry.get('timestamp'))
        except Exception:
            new_ts = None
        # Skip if interval < MIN_INTERVAL
