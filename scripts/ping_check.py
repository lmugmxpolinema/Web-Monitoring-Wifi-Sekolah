...
def update_ont_statuses():
    """Melakukan ping ke semua ONT dan mengupdate statusnya di data/onts.json."""
    try:
        ...
        temp_path = ONTS_FILE.with_name(".tmp-onts.json")
        ...
        try:
            with temp_path.open('w', encoding='utf-8') as tf:
                ...
                try:
                    ...
                    pass
            os.replace(temp_path, ONTS_FILE)
            print(f"Status ONT berhasil diperbarui di {ONTS_FILE}.")
        finally:
            try:
                if temp_path.exists():
                    temp_path.unlink()
    ...
    while True:
        try:
            ...
            if current_time - last_ping_check >= PING_INTERVAL:
                ...
                update_ont_statuses()
                # Rekam history jumlah ONT online ke server
                try:
                    with ONTS_FILE.open("r", encoding='utf-8') as f:
                        onts = json.load(f)
                    online_count = sum(1 for o in onts if o.get('status') == 'ON')
                    payload = {'timestamp': datetime.now().isoformat(), 'count': online_count}
                    requests.post(f"{FLASK_SERVER_URL}/api/record-ont-history", json=payload, timeout=5)
                    print(f"Recorded ONT online count: {online_count}")
                except Exception as e:
                    print(f"ERROR: Gagal merekam ONT history: {e}")
