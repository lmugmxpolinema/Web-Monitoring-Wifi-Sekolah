...
def load_csv_data():
    ...
    try:
        with CSV_SOURCE.open('r', encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
        print(f"File {CSV_SOURCE} tidak ditemukan")
        return []
...
def backup_existing_data():
    ...
    try:
        ...
        backup_filename = BACKUP_DIR / f'onts-backup-before-csv-{timestamp}.json'
        ...
        return len(existing_data)
            ...
            print(f"ℹ️  File {ONTS_FILE} tidak ditemukan, tidak ada backup yang dibuat")
        return 0
    ...
    try:
        ...
        try:
            if ONTS_FILE.exists():
                ...
                BACKUP_DIR.mkdir(parents=True, exist_ok=True)
                backup_filename = BACKUP_DIR / f'onts-backup-before-csv-{timestamp}.json'
                with ONTS_FILE.open('r', encoding='utf-8') as oldf, backup_filename.open('w', encoding='utf-8') as bf:
                    bf.write(oldf.read())
        ...
        os.replace(temp_path, ONTS_FILE)
            print(f"✓ Data baru tersimpan ke {ONTS_FILE}")
        return True
    except Exception as e:
...
def main():
    ...
    if save_new_data(converted_data):
        ...
        print(f"Data CSV ({len(csv_data)} item) berhasil dikonversi menjadi {len(converted_data)} item")
            print(f"File {ONTS_FILE} telah diperbarui dengan data dari CSV")
